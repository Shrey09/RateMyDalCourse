...
export class DashboardComponent implements OnInit { 
  ...
  */
    ...
    constructor(public authenticationService : AuthenticationService, public getCoursesService : GetCoursesService, private http : HttpClient, private router : Router, private route : ActivatedRoute) {}
    ...
    ngOnInit() { 
      // User should be authenticated when he access the dashboard
        ...
        this.getCoursesService.getMyCourses(this.usernameModel).subscribe(data => { // List of the courses completed by the user.
              ...
              // Fetching the string which user has entered in the searchbox
                ...
                for (var i in this.copyCourseSearchArray) { 
                  // https://www.google.com/search?q=trim+in+javascript&oq=trim+in+java&aqs=chrome.1.69i57j0l5.5473j0j4&sourceid=chrome&ie=UTF-8
                    if (this.copyCourseSearchArray[i].Name.toLowerCase().trim().includes(queryString.toLowerCase().trim()) || this.copyCourseSearchArray[i].Code.toLowerCase().trim().includes(queryString.toLowerCase().trim())) {
